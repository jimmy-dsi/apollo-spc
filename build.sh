...
# Build zig lib
"$SCRIPT_DIR/src/zig-build-lib.sh"
rm -f "$SCRIPT_DIR"/bin/*.o 2> /dev/null || :

# Copy resuling library to dotnet project folder
cp "$SCRIPT_DIR/bin/apollo.so" "$SCRIPT_DIR/src/cli/play/Apollo/apollo.so" 

# Build dotnet app
dotnet publish "$SCRIPT_DIR/src/cli/play/SpcProgram/SpcProgram.csproj" -r linux-x64 -p:PublishAot=true -p:WarningLevel=0 --output "$SCRIPT_DIR/bin/"

echo ""
echo "build.sh: Build successful"
echo "To run:"
echo "./bin/apollo-spc-program <your-spc-file>"
