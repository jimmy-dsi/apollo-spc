...
pub const SSMP = struct {
    ...
    inline fn exec_opcode(self: *SSMP, substate_offset: u32) !void {
        ...
        switch (opcode) {
            ...
            0x84 => { try self.spc.alu_a_with_d(substate_offset, SPC.AluOp.add);                        }, // adc a, dp
            0x85 => { try self.spc.alu_a_with_abs(substate_offset, SPC.AluOp.add);                      }, // adc a, addr
            0x86 => { try self.spc.alu_a_with_x_ind(substate_offset, SPC.AluOp.add);                    }, // adc a, (x)
            0x87 => { try self.spc.alu_a_with_d_x_ind(substate_offset, SPC.AluOp.add);                  }, // adc a, [dp+x]
            0x88 => { try self.spc.alu_a_with_imm(substate_offset, SPC.AluOp.add);                      }, // adc a, #im
            ...
            0x8D => { try self.spc.alu_y_with_imm(substate_offset, SPC.AluOp.mov);                      }, // mov y, #im
            0x8E => { try self.spc.pop_reg(substate_offset, &self.spc.state.psw);                       }, // pop psw
            ...
            0x94 => { try self.spc.alu_a_with_d_x(substate_offset, SPC.AluOp.add);                      }, // adc a, dp+x
            0x95 => { try self.spc.alu_a_with_abs_x(substate_offset, SPC.AluOp.add);                    }, // adc a, addr+x
            0x96 => { try self.spc.alu_a_with_abs_y(substate_offset, SPC.AluOp.add);                    }, // adc a, addr+y
            0x97 => { try self.spc.alu_a_with_d_ind_y(substate_offset, SPC.AluOp.add);                  }, // adc a, [dp]+y
            ...
            0x9C => { try self.spc.alu_modify_a(substate_offset, SPC.AluModifyOp.dec);                  }, // dec a
            0x9D => { try self.spc.mov_reg_reg(substate_offset, &self.spc.state.x, &self.spc.state.sp); }, // mov x, sp
            ...
            0xA4 => { try self.spc.alu_a_with_d(substate_offset, SPC.AluOp.sub);                        }, // sbc a, dp
            0xA5 => { try self.spc.alu_a_with_abs(substate_offset, SPC.AluOp.sub);                      }, // sbc a, addr
            0xA6 => { try self.spc.alu_a_with_x_ind(substate_offset, SPC.AluOp.sub);                    }, // sbc a, (x)
            0xA7 => { try self.spc.alu_a_with_d_x_ind(substate_offset, SPC.AluOp.sub);                  }, // sbc a, [dp+x]
            0xA8 => { try self.spc.alu_a_with_imm(substate_offset, SPC.AluOp.sub);                      }, // sbc a, #im
            ...
            0xAD => { try self.spc.alu_y_with_imm(substate_offset, SPC.AluOp.cmp);                      }, // cmp y, #im
            0xAE => { try self.spc.pop_reg(substate_offset, &self.spc.state.a);                         }, // pop a
            ...
            0xB4 => { try self.spc.alu_a_with_d_x(substate_offset, SPC.AluOp.sub);                      }, // sbc a, dp+x
            0xB5 => { try self.spc.alu_a_with_abs_x(substate_offset, SPC.AluOp.sub);                    }, // sbc a, addr+x
            0xB6 => { try self.spc.alu_a_with_abs_y(substate_offset, SPC.AluOp.sub);                    }, // sbc a, addr+y
            0xB7 => { try self.spc.alu_a_with_d_ind_y(substate_offset, SPC.AluOp.sub);                  }, // sbc a, [dp]+y
            ...
            0xBC => { try self.spc.alu_modify_a(substate_offset, SPC.AluModifyOp.inc);                  }, // inc a
            0xBD => { try self.spc.mov_reg_reg(substate_offset, &self.spc.state.sp, &self.spc.state.x); }, // mov sp, x
            ...
            0xC4 => { try self.spc.mov_d_from_a(substate_offset);                                       }, // mov dp, a
            0xC5 => { try self.spc.mov_abs_from_a(substate_offset);                                     }, // mov addr, a
            ...
            0xC8 => { try self.spc.alu_x_with_imm(substate_offset, SPC.AluOp.cmp);                      }, // cmp x, #im
            0xC9 => { try self.spc.mov_abs_from_x(substate_offset);                                     }, // mov addr, x
            0xCA => { try self.spc.mov1_mem_bit_with_c(substate_offset);                                }, // mov1 mem.b, c
            0xCB => { try self.spc.mov_d_from_y(substate_offset);                                       }, // mov dp, y
            0xCC => { try self.spc.mov_abs_from_y(substate_offset);                                     }, // mov addr, y
