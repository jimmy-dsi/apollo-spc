...
pub const SDSP = struct {
    ...
    pub fn proc(self: *SDSP, substate: u32) !void {
        // Main 32-step S-DSP loop. FullSNES indicates that this starts at step T0, but Ares source indicates that this starts at T1. Replicating Ares behavior.
        switch (substate) {
            0  => {
                //std.debug.print("Starting S-DSP sample loop\n", .{});
                //std.debug.print("DSP | Current cycle: {d}\n", .{self.cur_cycle()});
                self.proc_t1();  try self.co.wait(2);
            },
            1  => { self.proc_t2();  try self.co.wait(2); },
            2  => { self.proc_t3();  try self.co.wait(2); },
            3  => { self.proc_t4();  try self.co.wait(2); },
            4  => { self.proc_t5();  try self.co.wait(2); },
            5  => { self.proc_t6();  try self.co.wait(2); },
            6  => { self.proc_t7();  try self.co.wait(2); },
            7  => { self.proc_t8();  try self.co.wait(2); },
            8  => { self.proc_t9();  try self.co.wait(2); },
            9  => { self.proc_t10(); try self.co.wait(2); },
            10 => { self.proc_t11(); try self.co.wait(2); },
            11 => { self.proc_t12(); try self.co.wait(2); },
            12 => { self.proc_t13(); try self.co.wait(2); },
            13 => { self.proc_t14(); try self.co.wait(2); },
            14 => { self.proc_t15(); try self.co.wait(2); },
            15 => { self.proc_t16(); try self.co.wait(2); },
            16 => { self.proc_t17(); try self.co.wait(2); },
            17 => { self.proc_t18(); try self.co.wait(2); },
            18 => { self.proc_t19(); try self.co.wait(2); },
            19 => { self.proc_t20(); try self.co.wait(2); },
            20 => { self.proc_t21(); try self.co.wait(2); },
            21 => { self.proc_t22(); try self.co.wait(2); },
            22 => { self.proc_t23(); try self.co.wait(2); },
            23 => { self.proc_t24(); try self.co.wait(2); },
            24 => { self.proc_t25(); try self.co.wait(2); },
            25 => { self.proc_t26(); try self.co.wait(2); },
            26 => { self.proc_t27(); try self.co.wait(2); },
            27 => { self.proc_t28(); try self.co.wait(2); },
            28 => { self.proc_t29(); try self.co.wait(2); },
            29 => { self.proc_t30(); try self.co.wait(2); },
            30 => { self.proc_t31(); try self.co.wait(2); },
            31 => { self.proc_t0();    self.co.finish(2); },
            else => unreachable
        }
    }
