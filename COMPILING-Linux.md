# Compiling
This section details the build process on Linux for both the full **apollo-spc-program** app as well as the **apollo.so** library.
...
The CLI app utilizes utilizes dotnet's Ahead-Of-Time (AOT) compilation features. In order to build projects which use these features, you will need to install several dependencies—namely the clang compiler, among a few others. Installation command for Ubuntu or Debian based distros:
...
This will build the entire project as whole. This includes the Dotnet CLI app **apollo-spc-program**, as well as the standalone Zig library **apollo.so**.
...
```bash
bin/apollo-spc-program "<path-to-your-spc-file.spc>"
```

## Building the Library Only
If you do not wish to install the dotnet SDK onto your machine, you can choose to build **apollo.so** only (the Zig standalone library).

You can build the CLI app using the following command:

```bash
src/zig-build-lib.sh
```
